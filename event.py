...
class Event:
    ...
    def _update_all_events_file(self, path, old_date, new_date, new_title):
        ...
        if self.file_manager.file_exists(all_events_path):
            ...
            events = []
        events = [event for event in events if not (event.strip().startswith(old_date))]

        new_entry = f"{new_date} {new_title}"
    ...
    def delete_event(self, path, date):
        day, month, year = date.split("-")
        file_path = path + f"/{day}-{month}-{year}/{date}.json"
        ...
        if self.file_manager.file_exists(file_path):
            ...
            self._remove_event_from_all_events(path, date, title)
        else:
            raise FileNotFoundError("Event not found.")

    def _remove_event_from_all_events(self, path, date, title):
        all_events_path = path + "/all_events.txt"
        if self.file_manager.file_exists(all_events_path):
            events = self.file_manager.read_file(all_events_path).splitlines()
